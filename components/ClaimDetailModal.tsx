...
const CategoryIcon = ({ category }: { category: string }) => {
  switch (category) {
    case 'MODELS': return <Cpu className="w-5 h-5 text-blue-400" />;
    case 'CAPITAL': return <DollarSign className="w-5 h-5 text-emerald-400" />;
    case 'ENERGY': return <Zap className="w-5 h-5 text-yellow-400" />;
    case 'ROBOTICS': return <Activity className="w-5 h-5 text-orange-400" />;
    case 'SPACE': return <Rocket className="w-5 h-5 text-indigo-400" />;
    case 'BIOLOGY': return <Heart className="w-5 h-5 text-rose-400" />;
    case 'GOVERNANCE': return <Shield className="w-5 h-5 text-slate-300" />;
    case 'COMPUTE': return <Hash className="w-5 h-5 text-cyan-400" />;
    case 'INFRASTRUCTURE': return <Database className="w-5 h-5 text-zinc-400" />;
...
export const ClaimDetailModal = () => {
  ...
  const relatedClaims = useMemo(() => {
    ...
    return allClaims
      ...
      .filter(item => item.score >= 2) // PRD specified 2+ entities
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      {/* Slide-over Panel */}
      <div className="relative w-full sm:w-[500px] h-full bg-slate-950 border-l border-slate-800 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        
        {/* Header */}
        <div className="p-4 border-b border-slate-800 flex items-center justify-between shrink-0 bg-slate-900/50">
          <div className="flex items-center gap-3">
             <div className={`p-2 rounded-lg border ${CATEGORY_COLORS[selectedClaim.category] || 'bg-slate-800'}`}>
                <CategoryIcon category={selectedClaim.category} />
             </div>
             <div>
               <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">SIGNAL DETAILS</div>
               <div className="text-xs font-semibold text-slate-200">{selectedClaim.source_feed_name || 'Chronicle Source'}</div>
             </div>
          </div>
          <div className="flex items-center gap-1">
            {claimList.length > 1 && (
              <div className="flex items-center gap-1 mr-2 px-2 py-1 bg-slate-800/50 rounded-lg border border-slate-800 text-[10px] text-slate-500 font-mono">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <section className="space-y-4">
            <div className="flex items-center gap-2">
               <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase tracking-wider ${CATEGORY_COLORS[selectedClaim.category]}`}>
          ...
          {selectedClaim.metric_value && (
            <section className="bg-indigo-500/5 border border-indigo-500/20 rounded-xl p-5 shadow-inner">
               <div className="flex items-center gap-2 mb-3 text-indigo-400">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">MEASURED IMPACT</span>
               </div>
                  ...
                  {relatedClaims.map(rc => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded border uppercase tracking-tighter ${CATEGORY_COLORS[rc.category]}`}>
                           {rc.category}
                        </span>
                        <span className="text-[9px] text-slate-500">{new Date(rc.date).toLocaleDateString()}</span>
                      </div>
